...
from core.fresnel import set_layer_nk
...
def compute_wavelength_vs_RT_figures(
    ...
    """
    计算 R/T 随波长的变化，并绘制材料 n-k 随波长曲线。会就地修改 layers 的 nk（通过 set_layer_nk）。
    ...
    for i in range(len(wls)):
        for name, layer in zip(layer_names, layers):
            set_layer_nk(layer, nk_map[name][i], name)
...
def compute_TE_TM_wavelength_angle_figures(
    ...
    横坐标波长 (μm)，纵坐标角度 (deg)。会就地修改 layers 的 nk（通过 set_layer_nk）。
    ...
    for j in range(n_wl):
        for name, layer in zip(layer_names, layers):
            set_layer_nk(layer, nk_map[name][j], name)
