...
def upstream_openfilters_root() -> Path:
    """Sibling OpenFilters source tree (abeles + materials/*.mat)."""
    env = os.environ.get("OPENFILTERS_DIR", "").strip()
    if env:
        return Path(env).resolve()
    candidate = toykits_root().parent / "OpenFilters"
    if candidate.is_dir():
        return candidate
    return Path.home() / "repos" / "OpenFilters"
