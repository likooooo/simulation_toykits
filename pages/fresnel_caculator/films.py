from common import with_nk_columns, init_layer_config, ensure_fresnel_session_state, pyplot_fixed_width, save_result_mat_button, build_film_result_mat, get_available_materials
...
st.caption('**多层膜构建指令** — 📖 [使用说明](%s)' % _DOCS_URL)
...
col_cfg1, col_cfg2 = st.columns(2)
with col_cfg1:
    if st.button("🔄 构建", width="stretch", key="update film table"):
        if formula_str:
            try:
...
@st.fragment
def compute_and_show_result():
    """仅此片段在点击计算时重跑，避免整页控件抖动。"""
    ...
    if "film_result_cache" in st.session_state:
        ...
        # 一行 4 个指标，固定比例列，无嵌套，减少 reflow 导致的水平抖动
        te_r, te_t, tm_r, tm_t = st.columns([1, 1, 1, 1])
        with te_r:
            st.write("#### TE 模式")
            st.metric("Reflectance (R)", f"{result.R_s:.4f}")
        with te_t:
            st.write("&nbsp;")
            st.metric("Transmittance (T)", f"{result.T_s:.4f}")
        with tm_r:
            st.write("#### TM 模式")
            st.metric("Reflectance (R)", f"{result.R_p:.4f}")
        with tm_t:
            st.write("&nbsp;")
            st.metric("Transmittance (T)", f"{result.T_p:.4f}")
        # TE / TM 系数并排
        exp_te, exp_tm = st.columns(2)
        with exp_te:
            st.caption("TE fresnel coefficients")
            st.write(f"r: `{result.r_s:.4f}`")
            st.write(f"t: `{result.t_s:.4f}`")
        with exp_tm:
            st.caption("TM fresnel coefficients")
            st.write(f"r: `{result.r_p:.4f}`")
            st.write(f"t: `{result.t_p:.4f}`")
        pyplot_fixed_width(result.filmstack_fig)
        mat_bytes = build_film_result_mat(result, wl_display, angle_display)
        save_result_mat_button(mat_bytes, "film_result.mat", "films_save_mat")


compute_and_show_result()
