...
def test_ar_qw_formula_uses_db_materials(materials_db) -> None:
    ...
    formula = build_formula_for_preset(
        "ar_qw_si", FILMSTACK_PRESET_CATALOG, materials_db, wl_mid
    )
...
def test_bragg_period_conserved(materials_db) -> None:
    wl_mid = 0.55
    formula = build_formula_for_preset(
        "bragg_mirror", FILMSTACK_PRESET_CATALOG, materials_db, wl_mid
    )
    assert f"^{BRAGG_PERIOD_REPEAT}" in formula
    assert "(H " in formula
    match = re.search(
        rf"\(H ([0-9.]+) 1\.5 0\.0 air ([0-9.]+)\)\^{BRAGG_PERIOD_REPEAT}",
        formula,
    )
...
def test_paper_preset_inline_nk() -> None:
    formula = build_formula_for_preset(
        "paper_sio2_si", FILMSTACK_PRESET_CATALOG, {}, 0.633
    )
...
def test_spr_preset_inline_nk() -> None:
    formula = build_formula_for_preset(
        "spr_bk7_cr_au", FILMSTACK_PRESET_CATALOG, {}, 0.633
    )
