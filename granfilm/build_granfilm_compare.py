...
def resolve_granfilm_env(*, granfilm_root: Path | None = None) -> Path:
    """Validate GRANFILM_DIR and return resolved path."""
    root = Path(
        os.environ.get("GRANFILM_DIR", "").strip() or str(granfilm_root or DEFAULT_GRANFILM_DIR)
    ).resolve()
    testing = root / "testing"
    issues: list[str] = []
    if not root.is_dir():
        issues.append(f"GRANFILM_DIR 不存在: {root}")
    ...
    os.environ.setdefault("GRANFILM_DIR", str(root))
    return root
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help=f"GranFilm v1.0 root (default: GRANFILM_DIR or {DEFAULT_GRANFILM_DIR})",
    )
    parser.add_argument(
        "--write-legacy-viz",
        action="store_true",
    ...
    return build_granfilm_compare_ui(
        args.output,
        granfilm_root=args.granfilm_dir,
