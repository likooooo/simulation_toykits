...
from granfilm.common.baseline import default_baseline_path, load_baseline, parse_sphere_test_dat
...
def granfilm_case():
    if os.environ.get("GRANFILM_DIR", "").strip():
        return default_sphere_case(granfilm_dir=os.environ["GRANFILM_DIR"])
...
@pytest.mark.skipif(
    not (Path(os.environ.get("GRANFILM_DIR", "/home/like/repos/GranFilm-v1.0")) / "testing" / "SphereTest.dat").is_file(),
